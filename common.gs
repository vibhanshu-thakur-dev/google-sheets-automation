function insertCheckbox(activeRange){
...
function insertHyperlink(activeRange,inputString){
  let richValue = SpreadsheetApp.newRichTextValue()
   .setText(inputString.split("|")[0])
   .setLinkUrl(inputString.split("|")[1])
   .build();
  activeRange.setRichTextValue(richValue);
}

function generateSheets(currentSpreadSheet,sheetName){
  if(currentSpreadSheet.getSheetByName(sheetName) == null)
    return currentSpreadSheet.insertSheet(sheetName);
  else
  currentSpreadSheet.getSheetByName(sheetName)
}
